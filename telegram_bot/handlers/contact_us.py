...
from telegram_bot.helpers import escape, linked_student_from_user
from telegram_bot.keyboards.inline_keyboard import (
...
async def confirm_contact_send_callback(query, state):
    ...
    receiver_text = (
        ...
        f"From: <b>{escape(student_full_name)}</b>\n"
        f"Student ID: <b>{escape(student_login or '-')}</b>\n"
        f"Telegram: <b>{escape(username_text)}</b>\n"
        f"Telegram ID: <code>{query.from_user.id}</code>\n\n"
        f"Message to {escape(target['label'])}:\n"
    ...
    await query.message.answer(
        f"✅ Your message was sent to <b>{escape(target['label'])}</b>."
    )
