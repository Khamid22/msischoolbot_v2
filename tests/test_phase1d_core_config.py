def test_account_auth_flag_has_been_removed_from_core_config():
    import backend.core.runtime.config as config
...
def test_core_config_owns_runtime_settings_and_root_config_is_wrapper():
    ...
    core_source = Path("backend/core/runtime/config.py").read_text()
    ...
    assert "from backend.core.runtime.config import" in root_source
...
def test_account_auth_module_does_not_export_legacy_flag():
    import backend.modules.domains.identity.service as account_auth
