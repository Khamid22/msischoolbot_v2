...
def test_academics_timetable_and_announcement_query_modules_import_successfully():
    import backend.modules.organization.repository as organization_queries
    import backend.modules.academics.curriculum.repository as curriculum_queries
    import backend.modules.academics.groups.repository as group_queries
    import backend.modules.reporting.academic_repository as reporting_queries
    import backend.modules.communications.announcements_repository as announcement_queries
    import backend.modules.academics.timetable.repository as timetable_queries
...
def test_academic_services_use_module_repositories():
    group_service_source = Path("backend/modules/academics/groups/service.py").read_text()
    ...
    assert "from backend.modules.academics.groups import repository as group_repository" in group_service_source
    assert "from backend.modules.academics.timetable import repository as timetable_repository" in group_service_source
    ...
    assert "from backend.modules.reporting import academic_contract as academic_data" in dashboard_service_source
...
def test_targeted_academic_services_no_longer_embed_schema_sql():
    service_paths = [
        "backend/modules/organization/service.py",
        "backend/modules/academics/curriculum/service.py",
        "backend/modules/academics/groups/service.py",
        "backend/modules/academics/timetable/service.py",
...
def test_timetable_and_announcements_query_modules_use_migrated_schema():
    timetable_query_source = Path("backend/modules/academics/timetable/repository.py").read_text()
    announcement_query_source = Path("backend/modules/communications/announcements_repository.py").read_text()
    curriculum_query_source = Path("backend/modules/academics/curriculum/repository.py").read_text()
    group_query_source = Path("backend/modules/academics/groups/repository.py").read_text()
...
def test_hod_teacher_academy_scope_sql_lives_in_module_repository():
    scope_source = Path("backend/modules/teacher_academy/policies.py").read_text()
