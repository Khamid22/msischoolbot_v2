...
def test_academic_review_state_reads_and_marks_candidate_events():
    class Cursor:
        rowcount = 3
    ...
    assert "candidate_id = ANY(%s::bigint[])" in conn.calls[0][0]
    assert "SET read_at = COALESCE(read_at, now())" in conn.calls[1][0]
