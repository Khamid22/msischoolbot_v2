...
from web.backend.services.internal_dashboard import build_internal_overview_dataset
...
def _get_groups_from_db(school_filter="all"):
    """Return sorted list of group names from DB, optionally filtered by school."""
    try:
        with queries.connect_auth_db() as conn:
            if school_filter and school_filter != "all":
                rows = conn.execute(
                    """
                    SELECT DISTINCT g.name FROM academic_groups g
                ...
                rows = conn.execute(
                    "SELECT DISTINCT name FROM academic_groups WHERE lower(name) <> 'online' ORDER BY name"
                ).fetchall()
...
def _get_group_school_sets_from_db():
    ...
    try:
        with queries.connect_auth_db() as conn:
            rows = conn.execute(
                """
                SELECT g.name, s.code FROM academic_groups g
                JOIN academic_schools s ON s.id = g.school_id
