"""Compatibility shim — the canonical normalization rules live in ``shared.academics.canonical``.

This module re-exports the canonical helpers so existing
``web.backend.utils.normalization`` imports keep working. New code should import
from ``shared.academics.canonical`` directly.
...
from shared.academics.canonical import *  # noqa: F401,F403  (re-export the canonical surface)
