...
def init_storage():
    ...
    with _DB_LOCK:
        ...
        with _connect() as conn:
            ...
            from web.backend.services.announcements import ensure_announcements_schema
...
def list_students_for_admin(school_filter = _ADMIN_SCHOOL_FILTER_ALL):
    ...
    online_only_student_keys = {
        f"{str(row['school_name'] or _DEFAULT_SCHOOL_NAME).strip() or _DEFAULT_SCHOOL_NAME}|{_normalize_name(row['full_name'])}"
...
def _extract_auto_student_context(full_name):
    ...
    try:
        with _connect() as conn:
            enrollment = conn.execute(
                ...
                WHERE e.full_name_norm = ? AND e.active = 1
                LIMIT 1
                """,
            ...
            classmate_rows = conn.execute(
                ...
                JOIN academic_groups   g   ON g.id   = e.group_id
                JOIN academic_subjects sub ON sub.id = e.subject_id
                WHERE g.name = ? AND sub.name = ? AND e.active = 1
                  AND e.full_name_norm != ?
...
def get_admin_student_profile(student_row_id):
    ...
    full_name = str(row["full_name"]).strip()
    auto_context = _extract_auto_student_context(full_name)
...
def get_dashboard_student_profile(
    ...
    # Use DB for classmates (same group + subject, excluding self).
    auto_context = _extract_auto_student_context(full_name)
