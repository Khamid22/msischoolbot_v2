...
from web.backend.services.academic_postgres import ensure_academic_schema
from web.backend.admin.services.payment_service import (
...
def _subject_display_name(value):
    ...
    if normalized in {"m", "math", "mathematics", "igcse math a", "igcse mathematics a"}:
        return "Math"
    if normalized in {"eng", "english", "general english"}:
        return "English"
    if normalized in {"chem", "chm", "chemistry"}:
        return "Chemistry"
    if normalized in {"bio", "biology"}:
        return "Biology"
    if normalized in {"phy", "physics"}:
        return "Physics"
    return raw
...
def _date_sort_value(value):
    ...
    if len(parts) >= 2 and parts[0].isdigit() and parts[1].isdigit():
        ...
        if 1 <= day <= 31 and 1 <= month <= 12:
            now = datetime.utcnow()
            current_year = now.year - 1 if month > now.month + 1 else now.year
            try:
