"""Teacher SQL helpers backed by the current LMS schema.

DB-2 owns teacher query access here while the physical database schema remains
``msi_v2``. Legacy imports from ``database.queries.teacher_queries`` re-export
these functions during the transition.
"""
