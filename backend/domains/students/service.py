...
def list_students_for_admin(school_filter = canonical.ADMIN_SCHOOL_FILTER_ALL):
    ...
    with _connect() as conn:
        ...
        online_only_rows = queries.list_online_only_student_rows(conn)
...
def extract_auto_student_context(full_name, student_row_id=None):
    ...
    try:
        with _connect() as conn:
            enrollment = None
            if has_id:
                enrollment = queries.get_active_enrollment_for_student_row(
                    conn,
                    student_row_id,
                )
            if not enrollment and normalized_full_name:
                for row in queries.list_active_student_enrollments(conn):
            ...
            classmate_rows = queries.list_classmate_names(
                conn,
...
def get_student_subject_enrollments(public_dashboard_id):
    ...
    try:
        with _connect() as conn:
            ref = queries.get_student_ref_by_public_dashboard_id(conn, dashboard_id)
            if not ref:
                return []
            rows = queries.list_subject_enrollment_rows_for_student(
                conn,
                int(ref["student_id"]),
                int(ref["school_id"]),
            )
...
def list_enrolled_subject_options(student_id, school_code="", fallback_subject_name=""):
    ...
    try:
        with _connect() as conn:
            current = queries.get_student_ref_by_public_dashboard_id_and_school(
                conn,
                dashboard_id,
                school_code,
            )
            if not current:
                return fallback
            rows = queries.list_active_subject_options_for_student(
                conn,
...
def resolve_public_dashboard_for_student_row(student_row_id, *, preferred_group="", school_code=""):
    ...
    try:
        with _connect() as conn:
            rows = queries.list_public_dashboard_targets_for_student_row(
                conn,
