...
def insert_group(conn, school_id, program_id, group_name, group_code, legacy_group_id):
    conn.execute(
        """
        INSERT INTO msi_v2.groups (school_id, program_id, group_name, group_code, status, legacy_group_id)
        VALUES (%s, %s, %s, %s, 'active', %s)
        """,
        (int(school_id), int(program_id), group_name, str(group_code or ""), int(legacy_group_id)),
    )
...
def insert_student(conn, *, student_code, full_name, school_id, default_password, legacy_student_row_id):
    return conn.execute(
        """
        ...
        RETURNING id
        """,
        (student_code, full_name, int(school_id), default_password, int(legacy_student_row_id)),
    ).fetchone()


def upsert_student_auth(conn, student_id, password_hash):
    conn.execute(
        """
