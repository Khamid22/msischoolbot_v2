...
def _connect():
    return shared_queries.connect_auth_db()
...
def _ensure_account_invites_schema(conn):
    shared_queries.ensure_account_invites_schema(conn)
...
def _list_child_academic_indicators(conn, child_row):
    ...
    try:
        rows = parent_queries.list_parent_subject_indicator_rows(conn, student_row_id, full_name)
    except Exception:
        rows = []
...
def _list_child_recent_lessons(conn, child_row):
    ...
    full_name = str(child_row["full_name"] or "").strip()
    try:
        rows = parent_queries.list_parent_recent_lesson_rows(conn, student_row_id, full_name, limit=200)
    except Exception:
        rows = []
...
def link_parent_via_invite(
    ...
    with _connect() as conn:
        parent = parent_queries.link_parent_from_invite(
            conn,
        ...
        if parent and telegram_user_id:
            ...
            shared_queries.clear_student_telegram_user_conflicts(conn, parsed_telegram_user_id, -1)
            shared_queries.clear_admin_telegram_user_conflicts(conn, parsed_telegram_user_id)
            parent_queries.clear_parent_telegram_user_conflicts(
                conn,
...
def parent_from_telegram_user_id(telegram_user_id):
    ...
    if not parsed:
        return None
    with _connect() as conn:
        row = parent_queries.get_parent_by_telegram_id(conn, parsed)
...
def parent_children(parent_id):
    ...
    if not parsed:
        return []
    with _connect() as conn:
        rows = parent_queries.list_parent_client_child_rows(conn, parsed)
...
def create_parent_invite_code(token, student_row_id, issued_by=0, *, expires_days=14):
    ...
    with _connect() as conn:
        _ensure_account_invites_schema(conn)
        student_db_id = parent_queries.get_student_v2_id_by_legacy_row(conn, student_row_id)
        if student_db_id is None:
            raise ValueError("Selected student was not found.")
        staff_db_id = parent_queries.get_staff_db_id_for_admin_id(conn, issued_by)
        for _ in range(5):
            code = secrets.token_urlsafe(9).rstrip("=")
            try:
                parent_queries.insert_parent_invite_row(
                    conn,
                    token_hash=code,
                    token=token,
...
def get_parent_invite_token(code):
    ...
    with _connect() as conn:
        _ensure_account_invites_schema(conn)
        row = parent_queries.get_pending_parent_invite_token(conn, code)
...
def _list_invite_parent_accounts(conn):
    try:
        rows = parent_queries.list_invite_parent_rows(conn)
    except Exception:
        return []
...
def list_linked_parents_for_student(student_row_id):
    ...
    with _connect() as conn:
        rows = parent_queries.get_parents_for_student(conn, student_id)
...
def list_parent_children(parent_id):
    ...
    if not parent_id:
        return []
    with _connect() as conn:
        rows = parent_queries.list_parent_client_child_rows(conn, parent_id)
...
def assign_parent_child(parent_id, student_row_id):
    ...
    with _connect() as conn:
        parent = parent_queries.get_parent_exists_row(conn, parent_id)
        if not parent:
            raise ValueError("Parent account was not found.")
        student_v2_id = parent_queries.get_student_v2_id_by_legacy_row(conn, student_id)
        if student_v2_id is None:
            raise ValueError("Student was not found.")
        parent_queries.insert_parent_student_link(conn, parent_id, student_v2_id)
        conn.commit()
        rows = parent_queries.list_parent_client_child_rows(conn, parent_id)
        for row in rows:
...
def remove_parent_child(parent_id, student_row_id):
    ...
    with _connect() as conn:
        deleted = parent_queries.delete_parent_student_link(conn, parent_id, student_id)
        conn.commit()
...
def delete_parent_account(parent_id):
    ...
    with _connect() as conn:
        ...
        link_count = parent_queries.count_parent_child_links(conn, parent_id)
        if _safe_int(link_count["count"] if link_count else 0) > 0:
            raise ValueError("Unlink all students before deleting this parent.")

        ticket_count = parent_queries.count_parent_support_tickets(conn, parent_id)
        ...
        message_count = parent_queries.count_parent_ticket_messages(conn, parent_id)
        ...
        deleted = parent_queries.delete_parent_row(conn, parent_id)
        conn.commit()
...
def parent_can_access_student(parent_id, student_row_id):
    ...
    if not parsed_parent_id or not parsed_student_row_id:
        return False
    with _connect() as conn:
        row = parent_queries.get_parent_child_link(conn, parsed_parent_id, parsed_student_row_id)
    return bool(row)
...
def parent_can_access_dashboard(parent_id, dashboard_student_id):
    ...
    if not parsed_parent_id or not parsed_dashboard_student_id:
        return False
    with _connect() as conn:
        row = parent_queries.get_parent_child_link_by_dashboard_id(
            conn,
            parsed_parent_id,
