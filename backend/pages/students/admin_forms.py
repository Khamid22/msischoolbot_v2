...
from backend.core.request_context import current_app, request, session
...
from backend.services.teachers.core import assign_teacher_to_group, list_teachers
from backend.services.admin.workspace import invalidate_admin_page_context_cache
from backend.services.admin.compatibility import resolve_sheet_student_for_admin
...
def register_admin_student_routes(
    ...
    *,
    ...
    def admin_change_student_password_route(student_row_id: int):
        ...
        changed, change_error = admin_change_student_password(
            student_row_id,
            new_password,
            actor_account_id=session.get("account_id"),
        )
        if not changed:
            rendered = render_edit_student_page(
                student_row_id,
