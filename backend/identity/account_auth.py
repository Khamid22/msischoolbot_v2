...
This module reads the account tables and builds route-compatible session
payloads for account/profile password login.
"""
...
def build_legacy_session_payload(
    ...
    if role in STAFF_ACCOUNT_ROLES:
        ...
        if role == "system_admin":
            # Temporary compatibility: /admin routes still check
            # auth_role == "admin". Keep canonical role metadata while
            # allowing current admin screens to work.
            payload.update(
                {
                    "auth_role": "admin",
                    "admin_id": staff_id,
