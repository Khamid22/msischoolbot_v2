...
from backend.utils.context import RequestContextMiddleware, session, request
from backend.utils.demo_auth import is_demo_auth_enabled, maybe_apply_demo_auth
from backend.utils.normalization import normalize_text as _normalize, normalize_school_code as _normalize_school_code
...
def _compute_subject_rating(
    ...
    if not dashboards:
        from backend.domains.academics.internal_dashboard_service import get_subject_dashboards_from_db
...
def _load_dataset(school_code = None, force_refresh = False):
    from backend.domains.academics.internal_dashboard_service import build_internal_dataset
