from backend.platform.admin_page_cache import (
...
from backend.modules.reporting.academic_dashboard import build_internal_overview_dataset
from backend.modules.reporting.service import (
    list_active_group_names,
    list_active_group_school_mappings,
)

from backend.modules.reporting.insights import (
...
from backend.internal_operations.pages.options import (
...
def _get_groups_from_db(school_filter="all"):
    try:
        return list_active_group_names(school_filter)
    except Exception:
        return []
...
def _get_group_school_sets_from_db():
    ...
    try:
        rows = list_active_group_school_mappings()
        result = {}
        for group_name, school_code in rows:
            result.setdefault(group_name, set()).add(school_code)
        return result
    except Exception:
        return {}
...
def build_admin_page_context(
    *,
    ...
    if panel == "resources":
        ...
        admin_resource_subject_options = build_resource_subject_options(
            summary_rows,
            admin_resources,
        )
