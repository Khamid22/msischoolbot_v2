...
def _channel_chat_id(academy_teacher: dict[str, Any] | None) -> str:
    ...
    if subject_key:
        value = _text(os.getenv(subject_key))
        if value:
            return value
    return _text(os.getenv(TEACHER_ACADEMY_CHANNEL_ENV))


def _bot_token() -> str:
    return _text(os.getenv("BOT_TOKEN"))


def _portal_url() -> str:
    base_url = _text(os.getenv("MINI_APP_URL")).rstrip("/")
...
def _send_telegram_message(chat_id: Any, text: str) -> tuple[bool, str]:
    ...
    try:
        with request.urlopen(req, timeout=TELEGRAM_API_TIMEOUT_SECONDS) as response:
