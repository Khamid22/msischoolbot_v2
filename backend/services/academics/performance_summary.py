...
from backend.repositories import academics_summary as summary_repository
...
def list_subject_summaries_by_full_name(full_name):
    ...
    with _connect() as conn:
        rows = summary_repository.list_subject_summary_rows_by_full_name_norm(conn, normalized_name)
...
def list_subject_summaries(school_key=""):
    ...
    with _connect() as conn:
        rows = summary_repository.list_subject_summary_rows(
            conn,
...
def list_subject_student_counts():
    with _connect() as conn:
        rows = summary_repository.list_subject_student_count_rows(conn)
    return [
        {
...
def list_subject_group_counts():
    with _connect() as conn:
        rows = summary_repository.list_subject_group_count_rows(conn)
    return [
        {
            "subject_name": str(row["subject_name"] or "").strip(),
            "count": int(row["count"] or 0),
