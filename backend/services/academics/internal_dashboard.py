...
from backend.repositories import academics as academic_repository
...
def _academic_tables_exist(conn) -> bool:
    ...
    if _academic_tables_known_present:
        return True
    rows = academic_repository.list_msi_v2_table_names(conn)
...
def get_subject_dashboards_from_db(subject_name):
    ...
    with connect_auth_db() as conn:
        ...
        rows = academic_repository.list_subject_dashboard_rows(conn, subject_norm)
...
def build_internal_dataset(school_code=""):
    ...
    with connect_auth_db() as conn:
        ...
        rows = academic_repository.list_internal_dataset_enrollment_rows(conn, normalized_school)
        lesson_rows = academic_repository.list_internal_dataset_lesson_rows(conn, normalized_school)
        attendance_rows = academic_repository.list_internal_dataset_attendance_rows(conn, normalized_school)
        homework_rows = academic_repository.list_internal_dataset_homework_rows(conn, normalized_school)
        exam_rows = academic_repository.list_internal_dataset_exam_rows(conn, normalized_school)
...
def build_internal_overview_dataset(school_code=""):
    ...
    with connect_auth_db() as conn:
        ...
        enrollment_rows = academic_repository.list_internal_overview_enrollment_rows(conn, normalized_school)
        homework_rows = academic_repository.list_internal_dataset_homework_rows(conn, normalized_school)
        exam_rows = academic_repository.list_internal_dataset_exam_rows(conn, normalized_school)
        attendance_rows = academic_repository.list_internal_overview_attendance_rows(conn, normalized_school)
...
def get_enrollment_dashboard(public_dashboard_id, school_code="", subject_name="", group_name=""):
    ...
    with connect_auth_db() as conn:
        ...
        enrollment = academic_repository.get_enrollment_dashboard_row(
            conn,
        ...
        attendance_rows = academic_repository.list_enrollment_attendance_rows(
            conn,
            enrollment["group_id"],
        ...
        hw_rows = academic_repository.list_enrollment_homework_rows(
            conn,
            enrollment["group_id"],
            enrollment["student_id"],
        )
        ...
        exam_rows = academic_repository.list_enrollment_exam_rows(
            conn,
            enrollment["group_id"],
            enrollment["student_id"],
        )
