...
This is presentation-facing read composition. It depends on domain services;
other role modules must not depend on this workspace module.
"""

from backend.services.teachers.core import (  # noqa: F401
    assign_teacher_to_group,
    get_teacher_by_id,
    get_teacher_name_by_group,
    list_teachers,
    list_active_group_ids_for_teacher_group,
    update_teacher_by_id,
    upsert_teacher,
)
from backend.services.announcements.core import list_announcements
from backend.services.academics.operations import get_group_gradebook
from backend.services.teacher_academy.core import (
    get_academy_teacher_for_teacher_account,
)
from backend.services.teacher_academy.candidates import (  # noqa: F401
...
def build_teacher_workspace(teacher_id, staff_id=None):
    ...
    gradebooks = []
    if group_name:
        for group_id in list_active_group_ids_for_teacher_group(group_name):
            gradebook = get_group_gradebook(group_id)
            if gradebook:
