...
from backend.modules.customer_support import service
from backend.modules.customer_support.schemas import (
...
def _public_base_url(request: Request) -> str:
    ...
    if not proto:
        proto = "https" if str(request.headers.get("x-forwarded-ssl", "")).casefold() == "on" else "http"
...
def create_parent_invite(
    ...
    try:
        result = service.create_parent_invite(
            _actor(user), student_id, expected_version=payload.expectedVersion
        )
    except service.CustomerSupportError as exc:
    ...
    username = str(os.environ.get("TELEGRAM_BOT_USERNAME") or os.environ.get("BOT_USERNAME") or "").strip().lstrip("@")
