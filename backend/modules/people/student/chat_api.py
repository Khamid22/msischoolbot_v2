...
from backend.modules.domains.communications import contracts as chat_service
...
def _require_room_access(user: CurrentUser, room: str) -> None:
    if not chat_service.validate_room(room):
        raise HTTPException(status_code=400, detail="Invalid room.")
    if user.role != "student" or not user.student_db_id:
        raise HTTPException(status_code=403, detail="Student room access required.")
    if not chat_service.student_can_access_room(user.student_db_id, room):
        raise HTTPException(status_code=403, detail="You are not enrolled in this room.")
...
def list_messages(
    ...
    _require_room_access(user, room)
