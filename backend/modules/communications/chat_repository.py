...
def message_exists(conn, message_id: int):
    return conn.execute("SELECT id FROM msi_v2.chat_messages WHERE id = %s", (message_id,)).fetchone()


def upsert_blocked_student(conn, *, student_id, blocked_by, blocked_at, reason):
    conn.execute(
        """
        ...
        """,
        (student_id, blocked_by, blocked_at, reason),
    )


def delete_blocked_student(conn, student_id: str):
    conn.execute("DELETE FROM msi_v2.chat_blocked_users WHERE student_id = %s", (student_id.strip().lower(),))


def list_blocked_student_rows(conn):
    return conn.execute(
        """
        SELECT student_id, blocked_by_staff_login AS blocked_by_admin, blocked_at, reason
        FROM msi_v2.chat_blocked_users ORDER BY blocked_at DESC
        """
    ).fetchall()


def list_room_rows(conn):
    return conn.execute(
        """
        SELECT room, COUNT(*) AS total,
               SUM(CASE WHEN is_deleted IS FALSE THEN 1 ELSE 0 END) AS active
        FROM msi_v2.chat_messages GROUP BY room ORDER BY MAX(id) DESC
        """
    ).fetchall()
