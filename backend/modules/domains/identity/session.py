...
def set_account_session(auth_result):
    """Install a canonical, versioned account session into the current request."""
    ...
    if not isinstance(session_payload, dict):
        return False
    try:
        account_id = int(session_payload.get("account_id") or 0)
        session_version = int(session_payload.get("session_version") or 0)
    except (TypeError, ValueError):
        return False
...
def set_student_session(student, telegram_user_id=None):
    ...
    try:
        student_db_id = int(
            student.get("student_db_id")
            or student.get("canonical_student_id")
            or 0
        )
