...
# Temporary names retained for callers that imported the old module constants.
VALID_COMPLAINT_STATUSES = set(VALID_TICKET_STATUSES)
VALID_COMPLAINT_CATEGORIES = set(VALID_TICKET_CATEGORIES)
...
def _normalize_status(value):
    return normalize_ticket_status(value)


def _normalize_category(value):
    return normalize_ticket_category(value)
...
def _build_thread(conn, base):
    ...
    message_rows = [
        _message_from_row(r) for r in repository.list_complaint_message_rows(conn, base["id"])
    ]
    if message_rows:
