...
from backend.modules.domains.identity.service import provision_teacher_account
from backend.modules.domains.teacher_records import repository as repository
from backend.modules.domains.identity.database import DB_LOCK, connect, utc_now_iso
...
def backfill_teacher_auth(conn):
    ...
    if not missing:
        return
    now = utc_now_iso()
    repository.acquire_teacher_login_advisory_lock(conn)
    for row in missing:
