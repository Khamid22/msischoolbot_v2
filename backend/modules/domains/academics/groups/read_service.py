...
from backend.modules.domains.academics.calendar.service import serialize_calendar_closure
...
def list_group_page(*, school_id=0, subject_id=0, query="", cursor="", limit=50):
    ...
    with connect_auth_db() as conn:
        rows = group_repository.list_group_rows_page(
            conn,
            school_id=school_id,
...
def get_group_summary(group_id):
    ...
    with connect_auth_db() as conn:
        row = group_repository.get_group_summary_row(conn, parsed_group_id)
        if not row:
            return None
        summary = dict(row)
...
def list_program_page(*, cursor="", limit=50):
    ...
    page_offset = _offset(cursor)
    with connect_auth_db() as conn:
        rows = curriculum_repository.list_curriculum_program_page(
...
def list_program_item_page(program_id, *, cursor="", limit=100):
    ...
    page_offset = _offset(cursor)
    with connect_auth_db() as conn:
        rows = curriculum_repository.list_curriculum_item_page(
...
def list_timetable_range(*, start_date="", end_date="", group_id=0, school_id=0):
    ...
    with connect_auth_db() as conn:
        ...
        if int(group_id or 0) > 0:
            group = group_repository.get_group_by_legacy_or_id(conn, int(group_id))
            if not group:
...
def list_group_schedule_rows(group_id):
    ...
    if parsed_group_id <= 0:
        raise ValueError("Group is required.")
    with connect_auth_db() as conn:
        group = group_repository.get_group_by_legacy_or_id(conn, parsed_group_id)
        if not group:
            raise ValueError("Group was not found.")
        return [
            dict(row)
