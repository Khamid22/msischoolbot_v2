...
def lock_teacher_availability_scope(conn, teacher_id):
    conn.execute(
        "SELECT pg_advisory_xact_lock(hashtext(%s))",
        (f"msi_v2.office_hours.teacher:{int(teacher_id)}",),
    )


def teacher_has_active_subject(conn, teacher_id, subject_id):
...
def find_teacher_availability_overlap(conn, teacher_id, starts_at, ends_at):
    return conn.execute(
        """
        SELECT id
        FROM msi_v2.office_hour_slots
        WHERE teacher_id = %s
          AND status = 'open'
        ...
        LIMIT 1
        """,
        (int(teacher_id), ends_at, starts_at),
    ).fetchone()
