...
from backend.modules.organization.canonical import (
...
def compute_subject_rating(student_id, payload, dataset=None):
    ...
    if not dashboards:
        from backend.modules.reporting.academic_dashboard import get_subject_dashboards_from_db
...
def load_dataset(school_code=None, force_refresh=False):
    from backend.modules.reporting.academic_dashboard import build_internal_dataset
...
def load_dashboard_payload(
    ...
    from backend.modules.reporting.academic_dashboard import get_enrollment_dashboard
