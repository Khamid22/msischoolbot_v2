...
def get_curriculum_lesson_session_by_order(conn, group_id, lesson_order):
    return conn.execute(
        """
        ...
        LIMIT 1
        """,
        (int(group_id), int(lesson_order)),
    ).fetchone()


def get_group_lesson_session(conn, group_id, lesson_session_id):
    return conn.execute(
        """
        ...
        LIMIT 1
        """,
        (int(group_id), int(lesson_session_id)),
    ).fetchone()
