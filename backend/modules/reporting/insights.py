        ...
        metrics.append(
            {
                ...
                "student_key": str(row.get("student_row_id") or row.get("public_dashboard_id") or row.get("full_name", "")).strip(),
...
def build_admin_subject_info(metrics, dataset = None, school_option_catalog = None):
        ...
        for candidate in candidates:
            ...
            for fmt in (
                ...
                try:
                    ...
                    continue
                if parsed.year < 2020 or parsed.year > datetime.now(timezone.utc).year + 1:
                    continue
            ...
            if (
                ...
                year = int(parts[0])
                month = int(parts[1])
                if 1 <= month <= 12 and 2020 <= year <= datetime.now(timezone.utc).year + 1:
    ...
    for bucket_key, bucket in subject_buckets.items():
        ...
        if month_keys:
            ...
            month_range_values = []
        known_group_names = {
            str(group_row.get("label", "")).strip()
            for group_row in group_rows
            if str(group_row.get("label", "")).strip()
        }
        monthly_series = []
        monthly_group_names = sorted(
            known_group_names | set(monthly_subject_bucket),
            key=normalize_text,
        )
        for group_name in monthly_group_names:
            month_scores = monthly_subject_bucket.get(group_name, {})
            label = format_group_label(school_name, group_name)
            if not label:
                continue
            ...
            if has_value or month_range_values:
        ...
        monthly_ar_series = []
        monthly_ar_group_names = sorted(
            known_group_names | set(ar_subject_bucket),
            key=normalize_text,
        )
        for group_name in monthly_ar_group_names:
            month_ar_data = ar_subject_bucket.get(group_name, {})
            label = format_group_label(school_name, group_name)
            if not label:
                continue
            values = []
        ...
        exam_series = []
        exam_group_names = sorted(
            known_group_names | set(exam_subject_bucket),
            key=normalize_text,
        )
        for group_name in exam_group_names:
            exam_scores = exam_subject_bucket.get(group_name, {})
            label = format_group_label(school_name, group_name)
            if not label:
                continue
            values = []
...
def build_admin_subject_counts(metrics):
    subject_buckets = {}
    for item in metrics:
        subject_name = str(item.get("subject", "")).strip()
        if not subject_name:
            continue
        student_key = str(item.get("student_key", "")).strip()
        if not student_key:
            student_key = normalize_text(item.get("full_name", ""))
        if not student_key:
            continue
        subject_buckets.setdefault(subject_name, set()).add(student_key)

    rows = [
        {"subject_name": subject_name, "count": len(student_keys)}
        for subject_name, student_keys in subject_buckets.items()
    ]
