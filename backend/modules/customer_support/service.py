...
from backend.modules.customer_support import repository
from backend.modules.identity.service import provision_student_account, reset_student_password
...
def create_student(actor: SupportActor, payload: dict[str, Any]):
    ...
    with _connect() as conn:
        ...
        prefix = canonical.student_code_prefix(str(school["school_key"] or ""))
...
def create_parent_invite(
    ...
    with _connect() as conn:
        scope = load_scope(conn, actor)
        current = _ensure_student_visible(conn, scope, student_id)
        _ensure_version(current["version"], expected_version)
        _audit(
            conn,
            actor,
        ...
        conn.commit()
    return {"inviteCode": code, "studentId": student_id}
