...
WEBHOOK_CACHE_ENABLED = _is_truthy_env(
    "SHEETS_WEBHOOK_CACHE_ENABLED",
    default=bool(str(os.environ.get("GOOGLE_SHEETS_WEBHOOK_TOKEN", "")).strip()),
)
...
def _cache_is_fresh_for_school(school_code, now):
    cached_dataset = _CACHE_DATA.get(school_code)
    if not cached_dataset:
        return False

    if WEBHOOK_CACHE_ENABLED:
        if bool(_CACHE_DIRTY.get(school_code, False)):
            return False

        if WEBHOOK_MAX_STALE_SECONDS <= 0:
            return True

        updated_at = float(_CACHE_UPDATED_AT.get(school_code, 0))
        if updated_at <= 0:
            return False
        return (now - updated_at) < WEBHOOK_MAX_STALE_SECONDS

    cached_expires_at = float(_CACHE_EXPIRES_AT.get(school_code, 0))
    return now < cached_expires_at
...
def mark_school_dataset_dirty(school_codes = None, clear_cached_data = False):
    ...
    if school_codes is None:
        ...
        normalized_code = _normalize_school_code(school_codes)
    ...
    if not target_codes:
        target_codes = set(_CACHE_DATA.keys())

    with _CACHE_LOCK:
        for school_code in target_codes:
            _CACHE_DIRTY[school_code] = True
            _CACHE_EXPIRES_AT[school_code] = 0.0
            if clear_cached_data:
                _CACHE_DATA.pop(school_code, None)
                _CACHE_UPDATED_AT.pop(school_code, None)
...
def get_school_dataset(force_refresh = False, school_code = None):
    ...
    now = time.time()
    cached_dataset = _CACHE_DATA.get(normalized_school_code)
    if not force_refresh and _cache_is_fresh_for_school(normalized_school_code, now):
    ...
    with _CACHE_LOCK:
        ...
        try:
            dataset = _load_from_google_sheets(normalized_school_code)
        except SheetsDataError:
            # Keep serving stale cache when Google Sheets is temporarily unavailable.
            if cached_dataset:
                return cached_dataset
            raise
        _CACHE_DATA[normalized_school_code] = dataset
        _CACHE_UPDATED_AT[normalized_school_code] = now
        _CACHE_DIRTY[normalized_school_code] = False
        _CACHE_EXPIRES_AT[normalized_school_code] = now + CACHE_TTL_SECONDS
        return dataset
...
def _extract_google_error_details(exc):
    if isinstance(exc, HttpError):
        status_code = int(getattr(getattr(exc, "resp", None), "status", 0) or 0)
        message = ""
...
def _execute_google_request_with_retries(request_factory):
    last_exc = None
    for attempt in range(1, 4):
        try:
            ...
            last_exc = exc
            if attempt >= 3 or not _is_retryable_google_error(exc):
                break
            time.sleep(0.6 * attempt)
    raise last_exc


def _load_from_google_sheets(school_code):
    school_code = _normalize_school_code(school_code)
    ...
    try:
        metadata = _execute_google_request_with_retries(
        ...
        details = _extract_google_error_details(exc)
        raise SheetsDataError(
    ...
    for sheet_title in sheet_titles:
        group_info = _parse_group_info(sheet_title, school_code)
        if group_info:
    ...
    try:
        values_response = _execute_google_request_with_retries(
...
def _parse_sehriyo_group_info(sheet_title):
    ...
    class_name = _normalize_sehriyo_class_name(parts[0])
    ...
    normalized_code = f"{_legacy_sehriyo_class_key(class_name)}{subject_code}"
    return GroupInfo(
        title=title,
        normalized_code=normalized_code,
        subject_code=subject_code,
...
def _parse_group_rows(
    ...
    if _is_sehriyo_chemistry_group(group):
        exam_columns = list(range(2, 19))  # C..S
    else:
    ...
    coins_by_name = _extract_coins_by_name(rows, group)
    ...
    for row_number, row in enumerate(rows, start=1):
        ...
        average_grade_cell = _parse_numeric_score(
            _cell_value(row, _average_grade_column_index(group))
        )
        ...
        student_id = _build_stable_student_id(
            key=_build_student_row_key(group, full_name),
            used_student_ids=used_student_ids,
        )
        ...
        student = {
            ...
            "schoolName": _school_display_name(group.school_code),
            "coins": coins,
        }
...
def _build_homework_columns_metadata(
    ...
    is_sehriyo_chemistry = _is_sehriyo_chemistry_group(group)
    if is_sehriyo_chemistry:
        return _build_sehriyo_chemistry_homework_columns_metadata(
            rows,
            lesson_number_row,
            lesson_topic_row,
            lesson_date_row,
    ...
    for column_index in range(start_column, max_columns):
        ...
        if not topic:
            topic = "Topic"
        if _is_excluded_homework_session(label, topic):
            continue
...
def _build_sehriyo_chemistry_homework_columns_metadata(
    rows,
    lesson_number_row,
    lesson_topic_row,
    lesson_date_row,
    ...
    max_columns = max((len(row) for row in rows), default=0)
    lesson_start_column = 1  # B
    score_start_column = 20  # U

    metadata: list[dict[str, Any]] = []
    for lesson_column_index in range(lesson_start_column, max_columns):
        raw_label = _to_text(_cell_value(lesson_number_row, lesson_column_index))
        if not raw_label:
            continue
        ...
        score_column_index = score_start_column + (lesson_column_index - lesson_start_column)
        if score_column_index >= max_columns:
            continue

        raw_topic = _to_text(_cell_value(lesson_topic_row, lesson_column_index))
        if not raw_topic:
            raw_topic = _to_text(_cell_value(lesson_topic_row, score_column_index))

        raw_date = _cell_value(lesson_date_row, lesson_column_index)
        if raw_date is None or _to_text(raw_date) == "":
            raw_date = _cell_value(lesson_date_row, score_column_index)
        lesson_date = _format_lesson_date(raw_date)
...
def _average_grade_column_index(group):
    if _is_sehriyo_chemistry_group(group):
        return 19  # T
    return 20  # U


def _is_sehriyo_chemistry_group(group):
    normalized_school_code = _normalize_school_code(
...
def _is_excluded_homework_session(label, topic):
...
def _build_student_row_key(group, full_name):
...
def _extract_coins_by_name(rows, group):
