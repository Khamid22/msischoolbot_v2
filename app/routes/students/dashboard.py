...
def register_dashboard_routes(
    ...
    *,
    ...
    def dashboard(student_id):
        ...
        def _load_current_school_dataset(school_code = None, force_refresh_dataset = False):
        ...
        student_profile = get_dashboard_student_profile(
            ...
            load_dataset=_load_current_school_dataset,
        )

        dataset_for_subject_switch = dataset
        if not dataset_for_subject_switch:
            refreshed_dataset, load_error = _load_current_school_dataset(
    ...
    def api_student_dashboard(student_id):
        ...
        if payload_error:
            return jsonify({"message": payload_error}), 503
        if not payload:
