(function () {
  ...
  function createHomeworkChart() {
    ...
    const normalizeHomeworkLabel = (item, index) => {
      ...
      for (const candidate of candidates) {
        ...
        const lessonMatch = text.match(/(?:lesson\s*)?(\d{1,3})/i);
    ...
    const homeworkPoints = rawHomeworkGrades
      .map((item, index) => {
        ...
        return {
          score,
          label: normalizeHomeworkLabel(item, index),
        };
      })
      .filter((point) => point !== null);

    const scores = homeworkPoints.map((point) => point.score);
    ...
    const labels = homeworkPoints.map((point) => point.label);
    const scrollContainer = chartSvg.closest(".chart-scroll");
    const homeworkBox = chartSvg.closest(".chart-box-homework");
    const yAxisContainer = yAxisSvg ? yAxisSvg.parentElement : null;
    ...
    const visiblePoints = viewportProfile.isLaptop ? 20 : viewportProfile.isTablet ? 14 : 9;
    const axisWidth = Math.max(
      yAxisSvg && yAxisSvg.parentElement ? yAxisSvg.parentElement.clientWidth : 36,
      viewportProfile.isLaptop ? 46 : viewportProfile.isTablet ? 42 : 36
    );
    const paddingRight = viewportProfile.isLaptop ? 18 : 14;
    const paddingTop = viewportProfile.isLaptop ? 14 : 12;
    const paddingBottom = viewportProfile.isLaptop ? 40 : viewportProfile.isTablet ? 36 : 32;
    const yMin = 1;
    const yMax = 9;
    ...
    const minSpacing = viewportProfile.isLaptop ? 56 : viewportProfile.isTablet ? 48 : 40;
    const pointSpacing = Math.max(minSpacing, plotVisibleWidth / Math.max(visiblePoints - 1, 1));
    const fullWidth = Math.round(
      paddingRight + Math.max(0, labels.length - 1) * pointSpacing + 8
    );
    const minHeight = viewportProfile.isLaptop ? 280 : viewportProfile.isTablet ? 250 : 210;
    const fullHeight = Math.max(Math.round(homeworkBox.clientHeight || minHeight), minHeight);
    ...
    homeworkBox.style.setProperty("height", `${fullHeight}px`, "important");
    homeworkBox.style.setProperty("min-height", `${fullHeight}px`, "important");
    homeworkBox.style.setProperty("width", `${fullWidth}px`, "important");
    homeworkBox.style.setProperty("min-width", `${fullWidth}px`, "important");

    if (yAxisContainer) {
      yAxisContainer.style.setProperty("height", `${fullHeight}px`, "important");
      yAxisContainer.style.setProperty("min-height", `${fullHeight}px`, "important");
    }
    ...
    chartSvg.style.height = `${fullHeight}px`;
    ...
    if (yAxisSvg) {
      ...
      yAxisSvg.style.height = `${fullHeight}px`;
    ...
    const maxLabelChars = labels.reduce((maxLength, label) => {
      return Math.max(maxLength, String(label || "").length);
    }, 2);
    const computedLabelFontSize = Math.floor((pointSpacing * 0.88) / (maxLabelChars * 0.55));
    const labelFontSize = Math.max(
      viewportProfile.isLaptop ? 8 : 7,
      Math.min(viewportProfile.isLaptop ? 11 : 10, computedLabelFontSize)
    );
