        ...
        <ModalBody className="space-y-5">
          ...
          <section className="space-y-3" aria-labelledby="new-break-title">
            <div className="flex items-center justify-between gap-3">
              ...
              <span className="rounded-full bg-primary/8 px-2.5 py-1 text-[0.625rem] font-bold uppercase text-primary">{groupId ? "Group only" : "School-wide"}</span>
            </div>
            ...
            {preview ? <div className={`rounded-xl border p-3 ${preview.conflicts?.length ? "border-red-200 bg-red-50" : "border-primary/15 bg-primary/5"}`}><p className="text-sm font-black">Review impact</p><div className="mt-2 grid grid-cols-3 gap-2 text-center"><div><strong className="block text-lg tabular-nums">{preview.affectedGroupCount}</strong><span className="text-[0.625rem] text-muted-foreground">groups</span></div><div><strong className="block text-lg tabular-nums">{preview.affectedLessonCount}</strong><span className="text-[0.625rem] text-muted-foreground">lessons moved</span></div><div><strong className="block text-lg tabular-nums">{preview.preservedRecordedLessonCount}</strong><span className="text-[0.625rem] text-muted-foreground">records preserved</span></div></div>{preview.conflicts?.length ? <p className="mt-2 text-xs font-semibold text-red-700">{preview.conflicts[0]}</p> : <p className="mt-2 text-xs text-muted-foreground">Lesson IDs and academic records will not be recreated or deleted.</p>}</div> : null}
          </section>
