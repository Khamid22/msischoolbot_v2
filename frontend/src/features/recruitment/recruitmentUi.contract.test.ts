...
describe("compact recruitment pipeline", () => {
  ...
  test("reveals Trash Bin, Reject, and Candidate Withdraw targets during a drag", () => {
    ...
    assert.match(pipeline, /candidateOutcomeTargets = \["trash_bin", "rejected", "candidate_withdrew"\]/);
    assert.match(pipeline, /target === "rejected" \? "Reject" : "Withdraw"/);
    assert.match(pipeline, /flushSync\(\(\) => setDraggedCandidate\(value\)\)/);
    assert.match(pipeline, /createPortal\([\s\S]*document\.body/);
    assert.match(pipeline, /data-outcome-drop-target=\{target\}/);
    assert.match(pipeline, /z-\[90\]/);
...
describe("candidate navigation and progressive disclosure", () => {
  ...
  test("moves Academy and Active outcomes into a dedicated Teachers switcher", () => {
    ...
    const scopedRoster = projectSource("features/teacher-academy/ScopedTeacherAcademyRoster.tsx");
    const appointedLessons = projectSource("features/teacher-academy/AppointedLessonsView.tsx");
    const workflowModals = projectSource("features/teacher-academy/TeacherAcademyWorkflowModals.tsx");
    const academyModel = projectSource("features/teacher-academy/model.ts");
    const pipeline = source("PipelineView.tsx");
    ...
    assert.match(cards, /All subjects/);
    assert.match(roster, /teacher_subject/);
    assert.match(roster, /teacher_sort/);
    assert.match(roster, /teacher_page/);
    assert.match(academyModel, /if \(width < 768\) return 5/);
    assert.match(academyModel, /if \(width < 1280\) return 6/);
    assert.match(academyModel, /return 12/);
    assert.match(cards, /grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3/);
    assert.match(cards, /role="progressbar"/);
    assert.match(cards, /aria-valuenow/);
    assert.match(cards, /teacher\.primaryLabel/);
    ...
    assert.match(academyPanel, /<AppointedLessonsView/);
    assert.match(academyPanel, /export interface TeacherAcademyPanelProps/);
    assert.match(academyPanel, /mode: TeacherAcademyMode/);
    assert.match(academyPanel, /academyTeachers: AcademyTeacher\[\]/);
    assert.doesNotMatch(academyPanel, /state:\s*any|Record<string,\s*unknown>|#[0-9A-Fa-f]{3,8}/);
    ...
    assert.doesNotMatch(cards + appointedLessons, /#[0-9A-Fa-f]{3,8}/);
