...
function OutcomeFields({ candidate, options }: { candidate: RecruitmentCandidate; options?: RecruitmentOptions }) {
  const [decision, setDecision] = useState("on_hold");
  ...
  return (
    <>
      <label className="text-xs font-semibold">Decision<select name="decision" value={decision} onChange={(event) => setDecision(event.target.value)} className={`${fieldClass} mt-1`}><option value="on_hold">On Hold</option><option value="candidate_withdrew">Candidate Withdrew</option>{canFinalize ? <><option value="rejected">Rejected</option><option value="teacher_academy">Teacher Academy</option><option value="active_teacher">Active Teacher</option></> : null}</select></label>
      ...
      {decision === "rejected" ? <label className="text-xs font-semibold">Rejection reason<select required name="rejection_reason" className={`${fieldClass} mt-1`}><option value="">Select a reason</option>{options?.rejection_reasons.map((reason) => <option key={reason} value={reason}>{humanize(reason)}</option>)}</select></label> : null}
      {["rejected", "on_hold", "candidate_withdrew"].includes(decision) ? <label className="text-xs font-semibold">Reason / explanation<textarea name="reason_detail" required={decision === "on_hold" || decision === "rejected"} className={`${fieldClass} mt-1 min-h-24`} /></label> : null}
...
function ActionFields({ action, candidate, options }: { action: ProfileAction; candidate: RecruitmentCandidate; options?: RecruitmentOptions }) {
  switch (action.kind) {
    ...
    case "review_approval":
      return <div className="grid gap-3"><p className="rounded-lg bg-muted/50 p-3 text-sm">{stageLabels[text(action.approval.requested_outcome)]} approval will be {action.status}.</p><label className="text-xs font-semibold">Comment<textarea name="review_comment" required={action.status === "returned"} defaultValue={action.status === "approved" ? "Approved" : ""} className={`${fieldClass} mt-1 min-h-24`} /></label></div>;
    case "add_task":
...
function actionTitle(action: ProfileAction | null) {
  ...
  switch (action.kind) {
    ...
    case "review_approval": return action.status === "approved" ? "Approve request" : "Return request";
...
function actionSubmitLabel(action: ProfileAction | null) {
  ...
  if (action.kind === "review_approval") return action.status === "approved" ? "Approve" : "Return request";
...
export function CandidateProfile({ candidateId, basePath, role, onAnnouncement }: { candidateId: number; basePath: string; role: string; onAnnouncement: (message: string) => void }) {
  ...
  const backHref = origin === "pipeline" ? `${basePath}/pipeline` : origin === "tasks" ? `${basePath}/tasks` : `${basePath}/candidates${safeReturn}`;
  const backLabel = origin === "pipeline" ? "Pipeline" : origin === "tasks" ? "Tasks" : "Candidates";
  const backPath = origin === "pipeline" ? `${basePath}/pipeline` : origin === "tasks" ? `${basePath}/tasks` : `${basePath}/candidates`;
  ...
  if (permissions?.can_finalize || role === "hr_manager") hiringItems.push({ key: "outcome", label: "Record outcome", onClick: () => setAction({ kind: "record_outcome" }) });
