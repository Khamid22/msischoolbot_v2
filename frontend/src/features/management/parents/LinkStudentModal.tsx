...
import { asNumber, asString, getStudentCode, getStudentRowId } from "@/features/managementTypes";
...
export function LinkStudentModal({
  ...
  return (
    <div className="fixed inset-0 z-[75] flex items-center justify-center bg-foreground/60 p-4" onClick={onClose} role="presentation">
      <div
        ...
        <div className="flex shrink-0 items-center justify-between border-b border-foreground/8 px-4 py-3">
          <div className="flex min-w-0 items-center gap-2.5">
            ...
            <div className="min-w-0">
              <h3 id="link-student-title" className="truncate text-sm font-bold">Link a student</h3>
              <p className="truncate text-xs text-muted-foreground">to {parentDisplayName(parent)}</p>
            </div>
          </div>
          <button type="button" onClick={onClose} aria-label="Close link student dialog" className="flex h-11 w-11 items-center justify-center rounded-lg hover:bg-muted focus:outline-none focus-visible:ring-2 focus-visible:ring-primary/40">
            <X className="h-4 w-4" />
          </button>
        </div>
