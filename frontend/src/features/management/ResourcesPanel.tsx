...
import { asNumber, asString, sortSubjectsMathFirst, submitConfirm } from "@/features/managementTypes";
...
export default function ResourcesPanel({ state }: { state: any }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {addOpen ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/60 p-4" onClick={() => setAddOpen(false)} role="presentation">
          <div className="flex max-h-[90dvh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl bg-surface shadow-card-hover animate-in fade-in zoom-in-95 duration-150 motion-reduce:animate-none" onClick={(event) => event.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby="add-resource-title">
            <div className="flex shrink-0 items-center justify-between border-b border-foreground/5 px-5 py-3">
              <div>
                <h3 id="add-resource-title" className="text-sm font-bold">Add Resource</h3>
                <p className="text-xs text-muted-foreground">Use a file, an external URL, or both.</p>
              </div>
              <button type="button" onClick={() => setAddOpen(false)} className="flex h-11 w-11 items-center justify-center rounded-lg hover:bg-muted focus:outline-none focus-visible:ring-2 focus-visible:ring-primary/40" aria-label="Close add resource dialog">
                <X className="h-4 w-4" />
              </button>
            </div>
