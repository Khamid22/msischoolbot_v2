...
const fieldClass =
  "min-h-9 w-full rounded-lg border border-border bg-background px-2.5 py-1.5 text-sm text-foreground outline-none placeholder:text-muted-foreground focus-visible:border-primary focus-visible:ring-2 focus-visible:ring-primary/20";
...
function CircularProgress({ passed = 0, target = 0, label }: { passed?: number; target?: number; label: string }) {
  ...
  return (
    ...
    >
      ...
      <span className="absolute text-[0.6875rem] font-black tabular-nums text-foreground">
...
export function TeacherAcademyCard({ teacher }: { teacher: TeacherAcademyCardModel }) {
  ...
  return (
    <article className={`flex min-h-[15rem] flex-col rounded-xl border bg-card p-3 shadow-card transition-[border-color,box-shadow] duration-200 hover:border-primary/25 hover:shadow-card-hover motion-reduce:transition-none ${
      teacher.completed ? "border-success/35" : "border-border"
    }`}>
      <div className="flex min-w-0 items-start gap-3">
        <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg text-xs font-black shadow-sm ${avatarTone(teacher.fullName)}`} aria-hidden="true">
      ...
      <div className="mt-3 flex flex-wrap items-center gap-1.5">
        ...
        <span className="max-w-full truncate rounded-full bg-muted px-2 py-0.5 text-[0.625rem] font-bold text-muted-foreground" title={teacher.subject}>
          {teacher.subject}
        </span>
      </div>
      ...
      <dl className="mt-3 grid grid-cols-2 gap-3 border-t border-border pt-3">
        {isAcademy ? (
          <div className="min-w-0">
            <dt className="text-[0.625rem] font-black uppercase tracking-wide text-muted-foreground">Average score</dt>
          ...
          <div>
            <dt className="text-[0.625rem] font-black uppercase tracking-wide text-muted-foreground">Status</dt>
            <dd className="mt-1 text-sm font-black text-success">Active</dd>
          </div>
        )}
        <div className="min-w-0">
          <dt className="text-[0.625rem] font-black uppercase tracking-wide text-muted-foreground">
            {isAcademy ? "Joined" : "Active since"}
          </dt>
          <dd className="mt-1 break-words text-sm font-black text-foreground">{teacher.joinedLabel}</dd>
        </div>
      </dl>
      ...
      <div className="mt-auto flex items-center justify-between gap-2 pt-3">
        <button
          ...
          className="inline-flex min-h-11 items-center justify-center gap-2 rounded-lg border border-primary/25 bg-primary/10 px-2.5 py-1.5 text-[0.8125rem] font-semibold text-primary hover:bg-primary/15 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary/35 lg:min-h-9"
        >
...
export function TeacherCardGridSkeleton({ count = 6 }: { count?: number }) {
  return (
    <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3" aria-label="Loading teachers" aria-busy="true">
      {Array.from({ length: count }, (_, index) => (
        <div key={index} className="min-h-[15rem] animate-pulse rounded-xl border border-border bg-card p-3 shadow-card motion-reduce:animate-none">
          ...
          <div className="mt-3 h-6 w-2/3 rounded-full bg-muted" />
