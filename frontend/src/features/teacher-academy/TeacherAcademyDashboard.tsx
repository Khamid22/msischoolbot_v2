...
interface TeacherAcademyViewTabsProps {
  academyCount: number;
...
export function TeacherAcademyViewTabs({
  academyCount,
  activeTeacherCount,
  view,
  onViewChange,
}: TeacherAcademyViewTabsProps) {
  ...
  return (
    <nav
      ...
      className="no-scrollbar flex w-full max-w-full shrink-0 gap-1 overflow-x-auto rounded-xl border border-border bg-card p-1 shadow-sm xl:w-auto"
    >
        {tabs.map((tab, index) => {
          const Icon = tab.icon;
          ...
          return (
            <button
              ...
              aria-controls="academy-roster-results"
              tabIndex={active ? 0 : -1}
              onClick={() => onViewChange(tab.key)}
              onKeyDown={(event) => handleTabKeyDown(event, index)}
              className={`inline-flex min-h-11 min-w-[9rem] flex-1 items-center justify-center gap-2 rounded-lg px-3 py-1.5 text-[0.8125rem] font-semibold transition-colors duration-150 motion-reduce:transition-none lg:min-h-9 xl:flex-none ${focusClasses} ${
                active
