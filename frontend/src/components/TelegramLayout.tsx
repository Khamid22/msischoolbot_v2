...
export function Topbar({ backUrl, title, subtitle, subtitleContent, titleIcon, rightContent }: TopbarProps) {
  return (
    <header className="tg-fixed-topbar fixed inset-x-0 top-0 z-40 border-b border-foreground/5 bg-surface/95 shadow-card backdrop-blur">
...
export function TelegramLayout({ children, topbar }: TelegramLayoutProps) {
  return (
    <div className="tg-app-shell flex h-[100dvh] flex-col bg-background">
      {topbar}
      <main className="tg-app-main flex-1 overflow-y-auto overflow-x-hidden px-3 pt-[calc(var(--student-topbar-offset-mobile)+var(--safe-top))] sm:px-4 sm:pt-[calc(var(--student-topbar-offset-desktop)+var(--safe-top))] md:px-6 lg:px-8">
