...
export default function StudentOfficeHours(props: StudentOfficeHoursProps) {
  ...
  const content = (
    <div className="space-y-6">
      ...
      {bookingSlot && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/60 p-4" onClick={() => setBookingSlot(null)} role="presentation">
          <div className="flex max-h-[90dvh] w-full max-w-md flex-col overflow-hidden rounded-2xl bg-surface shadow-card-hover animate-in fade-in zoom-in-95 duration-150 motion-reduce:animate-none" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby="book-office-hour-title">
            <div className="flex shrink-0 items-center justify-between border-b border-foreground/5 px-5 py-3">
              <h3 id="book-office-hour-title" className="text-sm font-bold">Book Office Hour Session</h3>
              <button type="button" onClick={() => setBookingSlot(null)} className="flex h-11 w-11 items-center justify-center rounded-lg hover:bg-muted focus:outline-none focus-visible:ring-2 focus-visible:ring-primary/40" aria-label="Close booking dialog">
                <X className="h-4 w-4" />
              </button>
            </div>
